...
def color(distance, max_distance, k):
    x = int((max_distance - distance) * 255 / max_distance)
    if k < (WIDTH // 2 + HEIGHT // 2):
        l = x, 0, 0
    else:
        l = 0, x, 0
...
def main():
    ...
    while running:
        ...
        for i in circles.connect_circles():
            ...
            k = start_position[0] + start_position[1]

            if distance < max_distance:
                pg.draw.line(screen, color(distance, max_distance, k), start_pos=i[0], end_pos=i[1], width=2)
        ...
        for i in circles.circles:
            ...
            pg.draw.circle(screen, l, center=i[:2], radius=4)
